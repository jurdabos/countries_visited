...
class TestH5Utils:
    ...
    def test_get_palettes(self, mock_palettes_json):
        ...
        # Verify color info is present
        assert "_color_info" in palettes
        assert isinstance(palettes["_color_info"], dict)

        # Check that color info contains entries for the palette colors
        for color in palettes["Test Palette"]:
            assert color in palettes["_color_info"]
